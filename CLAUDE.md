...
- League config + snake-draft ordering live in `src/fantasy_football/leagues.py` and `config/leagues.yml`, deliberately separate from FantasyPros extraction. The shared player board is built once and reused for every league sheet — do not duplicate the player pipeline per league. Keeper logic is intentionally not implemented yet; it is the next planned iteration, so leave room for it rather than hard-coding assumptions that a pick always equals its slot.